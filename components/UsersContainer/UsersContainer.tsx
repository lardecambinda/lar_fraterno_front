...
import { getUsers, updateUser, deleteUser, toggleUserStatus } from "@/services/apolloAPI";
...
export default function UsersContainer() {
  ...
  const handleUpdate: SubmitHandler<IFormValues> = async (data) => {
    ...
    if (result) {
      toast("Usuário atualizado", { type: "success", theme: "light", hideProgressBar: true });
      setEditingUser(null);
      reset();
      loadUsers();
    }
  ...
  const handleToggle = async (id: string) => {
    ...
    if (result) {
      toast(`Usuário ${result.active ? "ativado" : "desativado"}`, { type: "success", theme: "light", hideProgressBar: true });
      loadUsers();
    }
  };
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
          ...
          <form className="space-y-4" onSubmit={handleSubmit(handleUpdate)}>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  className={`h-11 border rounded-md outline-none px-4 transition-colors ${
                    errors.user_name ? "border-rose-500" : "border-gray-300 focus:border-blue-500"
                  }`}
                  {...register("user_name", { required: true })}
                />
                {errors.user_name && <p className="text-rose-500 text-xs">Nome é obrigatório</p>}
              </div>
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  type="email"
                  className={`h-11 border rounded-md outline-none px-4 transition-colors ${
                    errors.email ? "border-rose-500" : "border-gray-300 focus:border-blue-500"
                  }`}
                  {...register("email", { required: true })}
                />
                {errors.email && <p className="text-rose-500 text-xs">Email é obrigatório</p>}
              </div>

              <div className="flex flex-col gap-1.5">
                <label className="text-sm font-medium text-gray-700">Perfil</label>
                <select className="h-11 border border-gray-300 rounded-md outline-none px-4 bg-white text-sm focus:border-blue-500" {...register("role")}>
            ...
            <div className="flex gap-2">
              ...
              <button type="button" onClick={handleCancelEdit} className="bg-gray-200 text-gray-700 px-6 py-2 rounded-md hover:bg-gray-300 transition-colors font-medium">
                Cancelar
              </button>
            </div>
          </form>
            ...
            <table className="w-full">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  {isAdmin && <th className="text-right px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider">Ações</th>}
                </tr>
              </thead>
                        ...
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <button 
                            ...
                            className={`inline-flex items-center gap-1 transition-colors ${
                              user.active ? "text-orange-600 hover:text-orange-800" : "text-green-600 hover:text-green-800"
                            }`}
