...
export default function NewUserPage() {
  ...
  const submit: SubmitHandler<IFormValues> = async ({ user_name, email, password, role }) => {
    setLoading(true);
    const result = await registerUser({ user_name, email, password, role });
    setLoading(false);
    if (result) {
      toast(`Usuário ${result.user_name} criado com sucesso`, { type: "success", theme: "light", hideProgressBar: true });
      reset();
      ...
      <div>
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">Cadastre um novo usuário no sistema</p>
          </div>
        </div>
      </div>
          ...
          <div className="space-y-5">
            <div className="flex flex-col gap-1.5">
              ...
              <input
                className={`h-11 border rounded-md outline-none px-4 transition-colors ${
                  errors.user_name ? "border-rose-500 focus:border-rose-500" : "border-gray-300 focus:border-blue-500"
                }`}
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input
                type="email"
                className={`h-11 border rounded-md outline-none px-4 transition-colors ${
                  errors.email ? "border-rose-500 focus:border-rose-500" : "border-gray-300 focus:border-blue-500"
                }`}
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input
                type="password"
                className={`h-11 border rounded-md outline-none px-4 transition-colors ${
                  errors.password ? "border-rose-500 focus:border-rose-500" : "border-gray-300 focus:border-blue-500"
                }`}
                placeholder="Mínimo 6 caracteres"
                {...register("password", { required: true, minLength: 6 })}
              />
              {errors.password && <p className="text-rose-500 text-xs">Senha mínima de 6 caracteres</p>}
            </div>
